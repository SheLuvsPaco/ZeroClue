...
static BASE: Lazy<Mutex<Url>> =
    Lazy::new(|| Mutex::new(Url::parse("http://172.20.10.11:8080").unwrap()));
...
fn app_data_dir() -> anyhow::Result<PathBuf> {
    ...
    // ✅ FIX: Use 'dirs' directly. This works on Android automatically.
    // We removed the broken 'tauri::api' call entirely.
...
async fn login(username: String, password: String, base_url: Option<String>) -> Result<String, String> {
    ...
    if let Some(base) = base_url {
        ...
        *BASE.lock().unwrap() = url;
        ...
        let url = Url::parse("http://172.20.10.11:8080").map_err(|e| {
...
async fn signup(username: String, password: String, base_url: Option<String>, invite_token: Option<String>) -> Result<String, String> {
    ...
    if let Some(base) = base_url {
        ...
        *BASE.lock().unwrap() = url;
        ...
        let url = Url::parse("http://172.20.10.11:8080").map_err(|e| format!("failed to set default base: {e}"))?;
        *BASE.lock().unwrap() = url;
    }
    ...
    if let Some(base) = base_url {
        ...
        *BASE.lock().unwrap() = url;
        ...
        if current_base.is_empty() {
            *BASE.lock().unwrap() = Url::parse("http://172.20.10.11:8080")
...
pub fn run() {
    // ✅ 1. VITE WAIT LOOP (The Fix for Blank Screen / No Logs)
    // We try to connect to the frontend port (1420) before launching the window.
    println!("[Rust] Checking if Vite dev server is ready...");
    
    let mut attempts = 0;
    let dev_port = 1420; // Default Tauri port. Change to 5173 if you use standard Vite.

    while attempts < 20 {
        match TcpStream::connect(format!("127.0.0.1:{}", dev_port)) {
            Ok(_) => {
                println!("[Rust] Vite is ready on port {}! Launching app...", dev_port);
                break;
            }
            Err(_) => {
                attempts += 1;
                if attempts % 5 == 0 {
                    println!("[Rust] Waiting for Vite... (Attempt {}/20)", attempts);
                }
    ...
    // ✅ 2. ENV VAR & CREDENTIALS (Your Existing Logic)
    // Auto-load base URL and credentials on startup
    if let Ok(base_str) = std::env::var("ZEROCHAT_BASE_URL") {
        println!("[Rust] Using Environment Base URL: {}", base_str);
        if let Ok(url) = Url::parse(&base_str) {
            *BASE.lock().unwrap() = url;
        }
    } else {
        // Default to localhost
        if let Ok(url) = Url::parse("http://172.20.10.11:8080") {
            *BASE.lock().unwrap() = url;
        }
    }
    ...
    // ✅ 3. BUILDER (Your Exact Configuration)
    tauri::Builder::default()
        .manage(AppState::default()) // <--- Preserved this!
